    ...
    return (
        <>
            <nav className='container mx-auto flex items-center justify-between py-4 px-6'>
                {/* Name of Website */}
                <div>
                    <Link to='/' className='text-2xl font-medium'>Shopaholic</Link>
                </div>

                {/* Links to Different Shopping Sections */}
                ...
                {/* Profile, Shopping Bag, Search, Hamburger (for mobile navigation) */}
                <div className='flex items-center space-x-4'>
                    ...
                    <button className='relative hover:text-black'>
                        <HiOutlineShoppingBag onClick={toggleCartDrawer} className='h-6 w-6 text-gray-700' />
                        {cartItemCount > 0 && (<span className='absolute -top-1 bg-brand text-white text-xs rounded-full px-2 py-0.5'>{cartItemCount}</span>)}
                    </button>
